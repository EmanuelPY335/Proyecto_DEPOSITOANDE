    ...
    <div className="fade-in">
        ...
        <div className="search-section">
          <div className="modern-search-bar">
            ...
            />
            {searchTerm && (<button className="clear-search-btn" onClick={() => setSearchTerm("")}><X size={16} /></button>)}
        ...
        <div className="table-container">
          <table className="styled-table">
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan="6" style={{ textAlign: "center", padding: "40px", color: "#666" }}><Loader2 className="animate-spin" size={24} style={{marginRight: 10}}/> Cargando...</td></tr>
              ) : filteredEmpleados.length > 0 ? (
                filteredEmpleados.map((e) => (
                  <tr key={e.id} className={ordenPendiente ? "row-highlight-mode" : ""}>
                    {/* IMPORTANTE: data-label agregado para versión móvil */}
                    ...
                    <td data-label="Depósito">{depositos.find((d) => d.ID_DEPOSITO === e.ID_DEPOSITO)?.NOMBRE || <span style={{color: '#999'}}>—</span>}</td>
                    <td data-label="Cargo"><span className="role-badge">{formatText(e.rol)}</span></td>
                    <td data-label="Estado"><span className={`status-badge ${e.estado ? "active" : "inactive"}`}>{e.estado ? "Activo" : "Inactivo"}</span></td>
                    <td data-label="Acción" style={{ textAlign: "center" }}>
                      {ordenPendiente ? (
                        <button className="btn-icon" onClick={() => handleAsignarOrden(e)} style={{color: '#d97706', background: '#fffbeb'}}><UserPlus size={20} /></button>
                      ) : (
                        <button className="btn-icon" onClick={() => setSelectedEmployee(e)}><MoreHorizontal size={20} /></button>
                      )}
                    </td>
                  </tr>
                ))

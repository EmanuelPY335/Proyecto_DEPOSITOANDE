...
const EmployeeModal = ({
  ...
  const stats = useMemo(
    () => ({
      ...
      faltas: 0, // luego si querés calculamos faltas/tardanzas con reglas
    }),
    [asistenciasArr]
  );
  ...
  const renderAsistenciaItem = (a) => {
    ...
    return (
      ...
      >
        ...
        <div style={{ marginTop: "0.35rem", fontSize: "0.75rem", color: "var(--gray-400)" }}>
          GPS: {a?.latitud ?? "—"}, {a?.longitud ?? "—"}
        </div>
      </div>
    );
  };

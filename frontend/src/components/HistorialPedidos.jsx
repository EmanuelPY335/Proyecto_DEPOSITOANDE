...
import { Package, FileText, ClipboardList, CheckCircle, XCircle, Eye } from "lucide-react";
...
const HistorialPedidos = ({ defaultView = "solicitudes", onAtenderPedido }) => {
  ...
  const renderSolicitudResumen = (sol) => {
    const items = Array.isArray(sol.items) ? sol.items : [];
    if (items.length === 0) return <span className="text-sm text-gray-400">Sin items</span>;
    ...
    return (
      <div className="text-sm">
        <div>
          <strong>{first.material}</strong>{" "}
 
        </div>
        {extra > 0 ? <div className="text-gray-400">+ {extra} más…</div> : null}
        {sol.observacion ? <div className="cell-obs">"{sol.observacion}"</div> : null}
      </div>
    );
  };
          ...
          <table className="discord-table historial-table">
            <thead>
              <tr>
                ...
                <th>{view === "solicitudes" ? "Materiales" : "Detalle Chofer"}</th>
                <th>Acciones</th>
              </tr>
            </thead>
            <tbody>
              {dataList.map((item) => (
                <tr key={item.id_solicitud || item.id}>
                  ...
                  <td>
                    {view === "solicitudes" ? (
                      <>
                        <strong>{item.deposito_solicitante}</strong>
                        <span className="cell-subtext">{item.solicitante_usuario}</span>
                      </>
                    ) : (
                      item.destino
                    )}
                  ...
                  <td>
                    {view === "solicitudes" ? (
                      renderSolicitudResumen(item)
                    ) : (
                      <span>
                        {item.chofer} <br />
                        <small className="text-gray-400">{item.vehiculo}</small>
                      </span>
                    )}
                  </td>
